...
async function fetchResult(taskId) {
  ...
  if (data.status !== 'done') {
    container.textContent = data.error || '任务失败，请查看任务状态和日志。';
  } else {
    const result = data.result || {}; const hits = result.top_hits || [];
    container.innerHTML = `<h2>最终候选 (${hits.length})</h2><p>报告: ${escapeHtml(result.reports?.report_html || '')}</p>` +
